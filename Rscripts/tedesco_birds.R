...
my_df <- read.csv(here('Data/lum_etal_2020_data/S4_Australian_amphibians.csv'))

# Appendix S3 indicates data analysed for 1979-2007
# but data goes back to 1758
my_df2 <-
  my_df %>% 
  filter(description_year > 1978)

# rescaling extinction years
last_ext = max(my_df2$extinction_year, na.rm = TRUE) - 1979

my_df2$description_year = ifelse(my_df2$description_year>my_df2$extinction_year, 
                                 my_df2$extinction_year-1, my_df2$description_year)

aus_amphs <- get_Tedesco_est(my_df2[2:3], last_ext)
